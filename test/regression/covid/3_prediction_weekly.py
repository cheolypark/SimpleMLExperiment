...
class Prediction(Config):
    ...
    def run_experiment_for_a_region(self, MAEs, start_date=26, period=14, test_province='Busan'):
        # 1 Select data
        ...
        for ml in self.algorithms:
            ...
            for day in range(start_date, start_date+period):
                # 6 Perform prediction
                y_predict = ml.predict(test_X)
                y_predict = y_predict.ravel()

                # 7 Add the predicted ratio
                df['ratio_predicted'] = y_predict

                # 8 Convert the predicted ratio to the confirmed and add them
                df['temp_pc'] = df['prev_confirmed_with_one'] * df['ratio_predicted']

                # Store the predicted confirmed
                df.at[day, predicted_confirmed] = df.iloc[day]['temp_pc']

                # 9 Create new ratio_confirmed after the starting date
                # 9.1 Add ratio between confirmed and prev_confirmed
                df.at[day, 'confirmed_with_one'] = df.iloc[day]['temp_pc']
                df["prev_confirmed_with_one"] = df["confirmed_with_one"].shift(1)

                df.at[day, "ratio_confirmed"] = df.iloc[day]['ratio_predicted']

                # 9.2 Update all previous ratio_confirmed (i.e., t-1, t-2, ...)
                for t in range(look_back_for_data):
                    # Update for result data
                    df[f"ratio_confirmed_t_{t+1}"] = df["ratio_confirmed"].shift(t+1).fillna(1)

                    # Update for test data
                    test_X[f"ratio_confirmed_t_{t+1}"] = df[f"ratio_confirmed_t_{t+1}"]

                print(day)

            # 10 Measure accuracy performance
            df['new_pc'] = df[predicted_confirmed].fillna(df['confirmed'])
            mae = mean_absolute_error(df['confirmed'], df['new_pc'])
            print(f'{name} \t', mae)
